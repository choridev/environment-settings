...
- **A Pane Is Always Framed**: `ui.pane_borders = "always"`, up from `"auto"`, which draws a border only once a pane has been split. With `"always"` a lone pane keeps the same edge as a split one, so the text does not shift sideways the moment a split appears or closes. Added in Herdr 0.9.0, and it does nothing unless `ui.pane_outer_borders` is on — it is, by default.
